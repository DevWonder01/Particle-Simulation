...
const int CELL_SIZE = 50;  // Adjust based on average particle size
...
public:
    SpatialGrid() : cols(GRID_COLS), rows(GRID_ROWS) {
        cells.resize(cols * rows);
    }

    void insert(RigidBody* body) {
        int cellX = body->sphere.getPosition().x / CELL_SIZE;
        int cellY = body->sphere.getPosition().y / CELL_SIZE;
        if (cellX >= 0 && cellX < cols && cellY >= 0 && cellY < rows) {
            cells[cellY * cols + cellX].push_back(body);
        }
    }

    void handleCollisionsInGrid() {
        for (auto& cell : cells) {
...
int main()
{
    ...
    std::uniform_real_distribution<> distrib_radius(2, 10);
    std::uniform_real_distribution<> distrib_mass(1, 10);
    std::uniform_int_distribution<> distrib_color(0, 255);

    SpatialGrid grid;  // Create the spatial grid object
